...
try{{
 ...
 if(location.hostname==='buildheritage.org'){{
  var _sid=sessionStorage.getItem('bh_sid');
  if(!_sid){{_sid=(Date.now().toString(36)+Math.random().toString(36).slice(2,8));sessionStorage.setItem('bh_sid',_sid);}}
  var _bh=function(t){{navigator.sendBeacon('https://bh-analytics.sunnyatlanta20.workers.dev/c',
   JSON.stringify({{t:t,p:location.pathname,r:document.referrer,l:_d.l||'',s:_sid}}));}};
  _bh('pv');
  var _fm=document.querySelector('form[action*="formsubmit"]');
  if(_fm)_fm.addEventListener('submit',function(){{_bh('lead');}});
 }}
}}catch(e){{}}
</script>"""
...
<p>When you request a quote we collect what you type into the form: your name, phone number, email address, city, the service you need, and any project details you share. The form also records basic context about your visit — the page that referred you to us, the page you arrived on, and the page you submitted from — so we know how people find us. We do not ask for — and you should not submit — payment details, government IDs, or other sensitive personal information. This site does not use tracking cookies. We do use cookieless, first-party analytics: each page view records the page visited, the referring site, approximate country, and device type (mobile/desktop) — no cookies, no cross-site tracking, no personal identifiers, and the data stays in our own systems.</p>
<h2>How we use it</h2>
<ul>
